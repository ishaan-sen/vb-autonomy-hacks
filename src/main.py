    ...
    if st.sidebar.button("Calculate Routes", type="primary"):
        with st.spinner("Calculating tactical routes..."):
            try:
                ...
                st.write(f"Debug: Start coordinates: {st.session_state.start_point}")
                st.write(f"Debug: End coordinates: {st.session_state.end_point}")
                ...
                try:
                    ...
                    st.write(f"Debug: Found start node: {start_node}")
                except Exception as e:
...
with col2:
    ...
    if 'routes' in st.session_state and 'current_graph' in st.session_state:
        ...
        for i, route in enumerate(st.session_state.routes):
            ...
            st.write(f"🔸 Route {i+1}: {len(route)} waypoints, {total_length:.0f}m")
